...
types = dir(img_root); types = types(3:end);

for i = 1:length(types)
   
    i
    videos = dir([ann_root types(i).name '/*.ann']);
    
    for j = 1:length(videos)
        
       ann = load([ann_root types(i).name '/' videos(j).name]);
       assert(size(ann,2) == 9);
       
       frameNames = dir([img_root types(i).name '/' videos(j).name(1:end-4) '/*.jpg']);
       ...
       for k = 1:size(ann,1) % number of annotated frames
           ...
           frameID = ann(k,1);
           im = imread([img_root types(i).name '/' videos(j).name(1:end-4) '/' frameNames(frameID).name]);
           x = min(ann(k,2:2:end));
           y = min(ann(k,3:2:end));
           ...
           fid = fopen(['/data01/ALOV/PosNegPerFrameALOV/' savefile], 'w');
       ...
       save([db_root 'PosNegPerFrameALOV_nr_ang16_r_step10_IoU_p0.7_n0.5/' videos(j).name(1:end-4) '.mat'], '-v7.3', 'num_samples');
