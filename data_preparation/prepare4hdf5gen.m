...
types = dir(img_root); 
types = types(3:end-1); % long-duration videos are not used.
...
%%
destRoot = '/data01/ALOV/val_metafile_for_gen_hdf5_trial01';
...
for i = 1:length(types)
    ...
    videos = dir([ann_root type_name '/*.ann']);
     ...
     for j = length(videos):length(videos)
        ...
        % alov ann file
        ann = load([ann_root type_name '/' videos(j).name]); assert(size(ann,2) == 9);
        frameNames = dir([img_root type_name '/' video_name '/*.jpg']); % all frames of the video
        ...
        for mm = 1:size(ann,1) % number of annotated frames
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            box_anno1 = load(['/data01/ALOV/PosNegPerFrameALOV/' type_name '_' video_name '_' imname1 '.txt']);
            ...
            for nn = mm+1:size(ann,1)
                ...
                %% box
                box_anno2 = load(['/data01/ALOV/PosNegPerFrameALOV/' type_name '_' video_name '_' imname2 '.txt']);
                ...
                num_pos_anno1 = length(find(box_anno1(:,end) > 0.5));
                num_pos_anno2 = length(find(box_anno2(:,end) > 0.5));

                boxes1(1:num_pos_anno1,:) = box_anno1(box_anno1(:,end) > 0.5, 2:end);
                boxes2(1:num_pos_anno2,:) = box_anno2(box_anno2(:,end) > 0.5, 2:end);
                ...
                box_anno1_ = box_anno1(num_pos_anno1+1:end,:);
                box_anno2_ = box_anno2(num_pos_anno2+1:end,:);

                % box_anno1_ = box_anno1_(box_anno1_(:,end) > 0.05,:);
                % box_anno2_ = box_anno2_(box_anno2_(:,end) > 0.05,:);
                ...
                randp1 = randperm(size(box_anno1_,1));
                boxes1(num_pos_anno1+1:min(num_boxes-num_pos_anno1,size(box_anno1_,1))+num_pos_anno1,:) = box_anno1_(randp1(1:min(num_boxes-num_pos_anno1,size(box_anno1_,1))),2:end);
                boxes1 = boxes1(1:min(num_boxes-num_pos_anno1,size(box_anno1_,1))+num_pos_anno1,:);
                
                randp2 = randperm(size(box_anno2_,1));
                boxes2(num_pos_anno2+1:min(num_boxes-num_pos_anno2,size(box_anno2_,1))+num_pos_anno2,:) = box_anno2_(randp2(1:min(num_boxes-num_pos_anno2,size(box_anno2_,1))),2:end);
                boxes2 = boxes2(1:min(num_boxes-num_pos_anno2,size(box_anno2_,1))+num_pos_anno2,:);
                
                % write to file
                if size(boxes1,1) < num_boxes || size(boxes2,1) < num_boxes
                    left_num = num_boxes * 2 - size(boxes1,1) - size(boxes2,1);
                    left_zeros = zeros(left_num, 5);
                    boxes = [boxes1;boxes2;left_zeros];
                else
                    boxes = [boxes1;boxes2];
                end
