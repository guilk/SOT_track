...
def get_symbol(num_labels, args):
    ...
    feat_norm = mx.symbol.L2Normalization(name='feat_norm', data=feat_l2)

    fc_dropout = mx.symbol.Dropout(data=feat_l2_fc6, name='fc_dropout')
    feat_dropout = mx.symbol.Dropout(data=feat_norm, name='feat_dropout')
    if args.lsoftmax:
        fc = mx.symbol.LSoftmax(data=fc_dropout, num_hidden=num_labels, beta=1000, margin=3, scale=0.99999, beta_min=5, name='lsoftmax')
    else:
        fc = mx.symbol.FullyConnected(data=fc_dropout, num_hidden = num_labels, name='cls_fc')
    softmax1 = mx.symbol.SoftmaxOutput(data=fc, use_ignore = True, ignore_label = 0, name='softmax', grad_scale=1.0)
    outputs = [softmax1]

    if args.lmnn:
        lmnn = mx.symbol.Custom(
            data=fc_dropout, epsilon=args.lmnn_epsilon, threshd = args.lmnn_threshd, grad_scale=0.0,
            op_type='lmnnLoss',name='lmnn')
        outputs.append(lmnn)
