...
function per_seperate_plot(seqs)
...
figPath = './figs/overall_seperate/';

perfMatPath = './perfMat/overall_seperate/';
...
rankingType = {'AUC', 'threshold'};%AUC, threshod
...
for i=1:length(metricTypeSet)
    ...
    if strcmp(metricType,'error')&strcmp(rankingType{i},'AUC')
        continue;
    end
    if strcmp(metricType, 'overlap') & strcmp(rankingType{i}, 'threshold')
        continue;
    end
    ...
    for j=1:length(evalTypeSet)
        ...
        dataName = [perfMatPath 'aveSuccessRatePlot_' seqs{1}.name '_' num2str(numTrk) 'alg_'  plotType '.mat'];
        
        % If the performance Mat file, dataName, does not exist, it will call
        % genPerfMat to generate the file.
        %if ~exist(dataName)
        genPerfMat_seperate(seqs, trackers, evalType, nameTrkAll, perfMatPath);
        %end
        ...
        figName= [figPath 'quality_plot_' seqs{1}.name '_' plotType '_' rankingType{i}];
