...
if __name__ == '__main__':
    ...
    model_dir = './models/threshold_0.5_lr_0.0001/'
    # model_dir = '../../models/SINT_pretrained/'
    epoch_index = 1
    gpu_id = 4

    saveRoot = '../../results/train_results/threshold_0.5_lr_0.0001/epoch_'+str(epoch_index)+'/'
    # saveRoot = '../../results/train_results/pretrain/'
    ...
    for vid in range(num_videos):
        ...
        for id in frameidx:
            ...
            img = preproc_img(im, mean, imageSz)
            model.forward(Batch([mx.nd.array(img), mx.nd.array(rois)]))
            tfeats = model.get_outputs()[0]
            # sub_feats = tfeats.asnumpy().squeeze()[:,25088:]
            # sub_feats = mx.nd.array(sub_feats, ctx=mx.gpu(gpu_id))

            # scores = mx.nd.dot(sub_feats, qfeat).asnumpy().squeeze()[:nr_samples]
