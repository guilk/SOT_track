...
if __name__ == '__main__':
    ...
    saveRoot = '../results/mxnet_epicflow/'
    ...
    for vid in range(num_videos):
        ...
        qfeat = model.get_outputs()[0].asnumpy().squeeze()
        ...
        for id in frameidx:
            ...
            tfeats = model.get_outputs()[0].asnumpy().squeeze()
            scores = np.dot(tfeats[:nr_samples,:], qfeat)
            ...
            if scores[max_idx]/float(base_sim) >= use_flow:
                # ################################################### flow
            ...
            ###################
            maxboxes[counter + 1, 1] = new_x
            maxboxes[counter + 1, 2] = new_y
            maxboxes[counter + 1, 3] = new_w + new_x - 1
            maxboxes[counter + 1, 4] = new_h + new_y - 1

            ## debug (conservative)
            # prev_box = samples[:, max_idx].copy()
            #
            # prev_box[2] = min(prev_box[2] - 1, im.shape[1] - 1)
            # prev_box[3] = min(prev_box[3] - 1, im.shape[0] - 1)
            #
            # prev_box[2] = prev_box[2] - prev_box[0] + 1  # w
            # prev_box[3] = prev_box[3] - prev_box[1] + 1  # h
